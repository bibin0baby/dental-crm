<template>
    <div>
      <!-- <Head :title="`${form.contact_id} `" /> -->
      <h1 class="mb-8 text-3xl font-bold">
        <Link class="text-indigo-400 hover:text-indigo-600" href="/contacts">Appointments</Link>
      ...
      <div class="max-w-3xl bg-white rounded-md shadow overflow-hidden">
        <form @submit.prevent="update">
          <div class="flex flex-wrap -mb-8 -mr-6 p-8">
            <text-input v-model="form.title" :error="form.errors.first_name" class="pb-8 pr-6 w-full lg:w-1/2" label="First name" />
            <text-input v-model="form.description" :error="form.errors.last_name" class="pb-8 pr-6 w-full lg:w-1/2" label="Last name" />
            <select-input v-model="form.contact_id" :error="form.errors.organization_id" class="pb-8 pr-6 w-full lg:w-1/2" label="Patient Name">
              <option :value="null" />
