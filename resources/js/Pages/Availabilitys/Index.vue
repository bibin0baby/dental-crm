<template>
  <div>
    ...
    <h1 class="mb-8 text-3xl font-bold">Doctor Schedule</h1>
  ...
  <Link class="btn-indigo" href="/availabilitys/create">
    ...
    <div class="bg-white rounded-md shadow overflow-x-auto">
      <table class="w-full whitespace-nowrap">
          ...
          <td class="border-t pb-7">
            <Link class="flex items-center px-6 py-4" :href="`/availabilitys/${availability.id}/edit`" tabindex="-1">
              ...
              <div v-if="availability.break_Fromtime">
                <span>{{ availability.break_Fromtime }} </span> <span>-</span> <span> {{ availability.break_Totime }}</span>
          ...
          <td class="border-t pb-7">
            <Link class="flex items-center px-6 py-4" :href="`/availabilitys/${availability.id}/edit`" tabindex="-1">
              ...
              <div v-if="availability.leave_FromDate">
                <span>{{ availability.leave_FromDate }} </span> <span>-</span> <span> {{ availability.leave_ToDate }}</span>
        ...
        <tr v-if="availabilitys.data.length === 0">
          <td class="px-6 py-4 border-t" colspan="4">No availability found.</td>
        </tr>
      </table>
    </div>
    <pagination class="mt-6" :links="availabilitys.links" />
  </div>
</template>
  ...
  watch: {
    form: {
      ...
      handler: throttle(function () {
        this.$inertia.get('/availabilitys', pickBy(this.form), { preserveState: true })
      }, 150),
    },
  },
  methods: {
