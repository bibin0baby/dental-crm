<template>
  <div>
    <Head title="Doctors List" />
    <h1 class="mb-8 text-3xl font-bold">Doctor Details</h1>
    <div class="flex items-center justify-between mb-6">
      ...
      <Link class="btn-indigo" href="/doctors/create">
        <span>Create</span>
    ...
    <div class="bg-white rounded-md shadow overflow-x-auto">
      <table class="w-full whitespace-nowrap">
        ...
        <tr v-for="user in users" :key="user.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
          <td class="border-t">
            <Link class="flex items-center px-6 py-4 focus:text-indigo-500" :href="`/doctors/${user.id}/edit`">
          ...
          <td class="w-px border-t">
            <Link class="flex items-center px-4" :href="`/doctors/${user.id}/edit`" tabindex="-1">
              <icon name="cheveron-right" class="block w-6 h-6 fill-gray-400" />
            </Link>
          </td>
        </tr>
        <tr v-if="users.length === 0">
          <td class="px-6 py-4 border-t" colspan="4">No doctors found.</td>
        </tr>
      </table>
    </div>
  </div>
